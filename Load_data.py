...
def load_Sigle_Three_Fmnist_test():
    a = 0
    if a == 1:
...
def Caltech101_20():
    ...
    # x4 = min_max_scaler.fit_transform(x4)   # or x4 = min_max_scaler.fit_transform(x4)/10
...
def BDGP():
    ...
    return [x1, x2], Y
def _load_npz():
    return np.load(path+"/rgbd.npz")
def _fix_labels(l):
    uniq = np.unique(l)[None, :]
    new = (l[:, None] == uniq).argmax(axis=1)
    return new
def load_dataset(to_dataset=False):
    npz = _load_npz()
    labels = npz["labels"]
    views = [npz[f"view_{i}"] for i in range(npz["n_views"])]
    views = [v.astype(np.float32) for v in views]
    # if to_dataset:
    #     dataset = th.utils.data.TensorDataset(*[th.Tensor(v).to(DEVICE, non_blocking=True) for v in views],
    #                                           th.Tensor(labels).to(DEVICE, non_blocking=True))
    # else:
...
def load_data_conv(dataset):
    print("load:", dataset)
    if dataset == 'Sigle_Three_Fmnist_Test':      # Fashion-10K-3views
        return load_Sigle_Three_Fmnist_test()
    elif dataset == 'MNIST_USPS_COMIC':           # MNIST-USPS
        return Get_MNIST_USPS_COMIC()
    elif dataset == 'Caltech101_20':              # Caltech101_20
        return Caltech101_20()
    elif dataset == 'BDGP':                       # BDGP
        return BDGP()
    elif dataset == 'ccv':                       # BDGP
        return ccv()
    elif dataset == 'rgbd':                       # BDGP
        return rgbd()
    elif dataset == 'voc':                       # BDGP
        return voc()
    else:
